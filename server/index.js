...
app.get("/api/me", authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ ...user, friendsList, notifications, servers: memberships });
  } catch (e) {
    console.error(e);
...
app.put('/api/me', authenticateToken, async (req, res) => {
  try {
    ...
    const updated = await prisma.user.update({
      ...
      data: { username, avatar, frame, banner }
    });
    res.json(updated);
  } catch (e) {
    console.error(e);
...
app.post("/api/shop/buy", authenticateToken, async (req, res) => {
  try {
    ...
    const updated = await prisma.user.update({
      where: { id: user.id },
      data: { coins: { decrement: price }, inventory: JSON.stringify(inventory) }
    });
  ...
  try {
    ...
    const created = await prisma.server.create({
      data: {
        ...
        ownerId: req.user.id,
        channels: { create: [{ name: "general", type: "text" }, { name: "voice", type: "voice" }] },
...
app.get("/api/server/:id", authenticateToken, async (req, res) => {
  try {
    ...
    const serverData = await prisma.server.findUnique({
      where: { id },
      include: { channels: true, owner: true, members: { include: { user: true } } },
    });
...
app.post("/api/servers/invite", authenticateToken, async (req, res) => {
  try {
    ...
    const notif = await prisma.notification.create({
      data: { type: "SERVER_INVITE", senderId: req.user.id, receiverId: receiver.id, serverId: sid, status: "PENDING" },
      include: { sender: true, server: true },
    });
...
app.post("/api/friends/invite", authenticateToken, async (req, res) => {
  try {
    ...
    if (receiver.id === req.user.id) return res.status(400).json({ error: "Self" });
    const me = await prisma.user.findUnique({ where: { id: req.user.id } });
    const list = safeJsonParse(me?.friendsData || "[]", []);
    if (list.includes(receiver.id)) return res.json({ ok: true });
    const existing = await prisma.notification.findFirst({ where: { type: "FRIEND_REQUEST", senderId: req.user.id, receiverId: receiver.id, status: "PENDING" } });
    if (existing) return res.json({ ok: true });
    const notif = await prisma.notification.create({
      data: { type: "FRIEND_REQUEST", senderId: req.user.id, receiverId: receiver.id, status: "PENDING" },
      include: { sender: true, server: true },
    });
    io.to(`user_${receiver.id}`).emit("new_notification", notif);
    res.json({ ok: true });
...
app.post("/api/notifications/respond", authenticateToken, async (req, res) => {
  try {
    ...
    if (!notif || notif.receiverId !== req.user.id) return res.status(403).json({ error: "Forbidden" });
    if (action === "DECLINE") {
...
app.put("/api/messages/:id", authenticateToken, async (req, res) => {
  try {
    ...
    const updated = await prisma.message.update({ where: { id }, data: { content, isEdited: true }, include: { user: true, reactions: { include: { user: true } } } });
...
app.post("/api/messages/:id/react", authenticateToken, async (req, res) => {
  try {
    ...
    const existing = await prisma.reaction.findFirst({ where: { messageId: id, userId: req.user.id, emoji } });
    if (existing) { await prisma.reaction.delete({ where: { id: existing.id } }); }
    else { await prisma.reaction.create({ data: { emoji, userId: req.user.id, messageId: id } }); }
    const updated = await prisma.message.findUnique({ where: { id }, include: { user: true, reactions: { include: { user: true } } } });
    const room = updated.channelId ? `channel_${updated.channelId}` : updated.dmRoom;
    if (room) io.to(room).emit("message_updated", updated);
    res.json({ ok: true });
  } catch (e) {
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", async () => {
    try {
      ...
      if (socket.userId) {
        ...
        io.emit("user_status_changed", { userId: socket.userId, status: "offline", lastSeen: new Date().toISOString() });
      }
    } catch (e) { console.error(e); }
  });
  ...
  socket.on("join_channel", async ({ channelId }) => {
    try {
      ...
      socket.join(room);
      const history = await prisma.message.findMany({ where: { channelId: Number(channelId) }, orderBy: { createdAt: "asc" }, include: { user: true, reactions: { include: { user: true } } } });
  ...
  socket.on("join_dm", async ({ roomName }) => {
    try {
      ...
      const history = await prisma.message.findMany({ where: { dmRoom: String(roomName) }, orderBy: { createdAt: "asc" }, include: { user: true, reactions: { include: { user: true } } } });
      socket.emit("load_history", history);
    } catch (e) { console.error(e); }
  });

  socket.on("typing", ({ room }) => { if (socket.userId) socket.to(room).emit("user_typing", socket.userId); });
  socket.on("stop_typing", ({ room }) => { if (socket.userId) socket.to(room).emit("user_stop_typing", socket.userId); });
  ...
  socket.on("request_voice_states", async () => {
    try { for (const roomId of Object.keys(rooms)) await broadcastRoomUsers(roomId); } catch (e) {}
  });
  ...
  socket.on("join_voice_channel", async (roomId) => {
    try {
      ...
      rooms[rid].filter((p) => p.socketId !== socket.id).forEach((p) => io.to(p.socketId).emit("user_joined_voice", { callerID: socket.id, signal: null }));
      await broadcastRoomUsers(rid);
    } catch (e) { console.error(e); }
  });

  socket.on("leave_voice_channel", async () => {
    try { await leaveRoomBySocket(socket); } catch (e) { console.error(e); }
  });
